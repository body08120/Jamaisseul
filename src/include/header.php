<!--=================================
 preloader -->

<div id="pre-loader">
    <img src="assets/images/pre-loader/loader-01.svg" alt="loader">
</div>

<!--=================================
 preloader -->

<!--=================================
 header -->

<header id="header" class="header default">

    <!--=================================
 mega menu -->

    <div class="menu">
        <!-- menu start -->
        <nav id="menu" class="mega-menu">
            <!-- menu list items container -->
            <section class="menu-list-items">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 position-relative">
                            <!-- menu logo -->
                            <ul class="menu-logo">
                                <li>
                                    <a href="index.php"><img id="logo_img" src="assets/img/logo01.png" alt="logo"
                                            height="100px"> </a>
                                </li>
                            </ul>
                            <!-- menu links -->
                            <div class="menu-bar">
                                <ul class="menu-links">
                                    <li><a href="index.php">Accueil</a></li>
                                    <li><a href="index.php?action=Information"> Qui Sommes-Nous <i
                                                class="fa fa-angle-down fa-indicator"></i></a>
                                        <!-- drop down multilevel  -->
                                        <ul class="drop-down-multilevel">
                                            <li><a href="index.php?action=Information#histo">Historique</a></li>
                                            <li><a href="index.php?action=Information#val">Nos Valeurs</a></li>
                                            <li><a href="index.php?action=Information#orga">Notre Organisation</a></li>
                                        </ul>
                                    </li>
                                    <li><a href="index.php?action=Services"> Nos établissements et services <i
                                                class="fa fa-angle-down fa-indicator"></i></a>
                                        <!-- drop down multilevel  -->
                                        <ul class="drop-down-multilevel">
                                            <li><a href="index.php?action=Hebergement">Pôle Hébergement</a></li>
                                            <li><a href="index.php?action=MedicoSocial">Pôle Médico-Social et Logement
                                                    Adapté</a></li>
                                            <li><a href="index.php?action=Asile">Pôle Asile</a></li>
                                        </ul>
                                    </li>
                                    <li><a href="index.php?action=Actualites">Nos Actualités</a></li>
                                    <li><a href="index.php?action=Recrutements">Recrutement</a></li>
                                    <li><a href="index.php?action=Contact">Contact</a></li>


                                </ul>

                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </nav>
        <!-- menu end -->
    </div>
</header>

<!--=================================
 header -->